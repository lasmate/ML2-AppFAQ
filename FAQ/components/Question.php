...
foreach ($faqdata as $row) {
    ...
    $IDuser[] = $row['id_user'];
    $IDuserR[] = $row['id_user_R'];
    $valider[] = $row['Valider'];
}
echo "<div>";
for ($i = 0; $i < count($faqdata); $i++) {
    if ($valider[$i]==0 && $_SESSION["id_usertype"] =='3') {
        console_log("question non validée");    }
    else{
        ...
        if (empty($reponses[$i])) { // Gestion des erreurs s'il n'y a pas de réponse
            ...
            echo "<p>Répondu par : " . htmlspecialchars($IDuser[$i], ENT_QUOTES) . "</p></div>";
        }
